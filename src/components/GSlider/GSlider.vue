    ...
    setup(props, context) {
      ...
      function genLabel() {
        if(!props.label) return null;
          ...
          return <span class="g-slider-label">{props.label}</span>
      }
      ...
      const wrapperClass = computed(() => ({
        'g-slider-wrapper': true,
        'g-slider-wrapper__vertical': props.vertical
      }))

      function genSliderWrapper() {
        return <div class={wrapperClass.value}>
          {genLabel()}
          {genSlider()}
        </div>
      }
