						...
						>
							<slot name="prepend" :item="item" :isSelected="isActiveItem(item)">
								<div :class="prependClasses" v-if="item.prepend &&  prependType">
									<g-icon @click="console.log('icon clicked')" v-if="prependType==='icon'">{{item.prepend}}</g-icon>
...
<script>
  ...
  export default {
    ...
    setup: function (props, context) {
      ...
      const renderList = computed(() => props.items && props.items.filter(item => item[props.itemTitle]))
