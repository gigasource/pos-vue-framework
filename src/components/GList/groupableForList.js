...
export function groupableForList(props, vModel) {
  ...
  const toggleItem = (item) => {
    if (multiple) {
      ...
      if (returnObject ) {
        ...
        if(itemValue) {
          updateSingle(item[itemValue])}
        else if(!itemValue && !itemText) {
          updateSingle(item);}
  ...
  const isActiveItem = (item) => {
    ...
    if (returnObject) {
      ...
      return itemValue ? item[itemValue] === vModel.value : (uniqueItems.value.indexOf(item) === vModel.value || vModel.value === item)
    }
  };
  ...
  const convertValueToInternalValue = function (value) {
    ...
    if (!Array.isArray(props.value)) {
      if (props.returnObject) {
        return props.itemValue ? props.items.find(i => i[props.itemValue] === value): props.items.find(i => _.isEqual(i, value));
      } else {
        return props.items.find(i => i[props.itemValue] === value)[props.itemValue];
      }
    }
    if (props.returnObject) {
      return props.itemValue ? props.items.filter(i => value.some(el => el === i[props.itemValue])): null
