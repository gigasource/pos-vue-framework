<script>
  ...
  import { Length, Angle, Color, LineStyle, FontFamily, FontWeight, cssLineStyleList, cssFontFamilyList, cssFontStyleList } from './GCssCustomizerHelper';
    ...
    setup(props, context) {
      ...
      const defaultDesignData = {
        ...
        text: {
          ...
          fontWeight: new FontFamily('Normal'),
          lineHeight: new Length(22),
          letterSpacing: new Length(0, '%'),
          textAlign: 'Left',
          align: 'Top',
        },
        fill: {
      ...
      const designPropertyList = ref({
        ...
        text: [
          { key: 'fontFamily', cssKey: 'fontFamily', items: [] },
          { key: 'fontWeight', appendInnerIcon: 'icon-arrow_drop_down', cssKey: 'fontStyle' },
          { key: 'fontSize', cssKey: 'fontSize', constructor: Length },
          { key: 'lineHeight', prependInnerIcon: 'icon-tops_line_height', cssKey: 'lineHeight', },
          { key: 'letterSpace', prependInnerIcon: 'icon-tops_letter_spacing', cssKey: 'letterSpacing', }
      ...
      const testItems = []
      const genTextSectionCombobox = () => {
        return <div claas="g-css-customizer-design-panel-basic-input-wrapper__long">
          <g-checkbox/>
          <g-css-customizer-combobox items={testItems}/>
        </div>
      }
      const testItems2 = []
      const genTextSectionSelect = () => {
        return <div claas="g-css-customizer-design-panel-basic-input-wrapper__short">
          <g-checkbox/>
          <g-css-customizer-select items={testItems2}/>
        </div>
      }
      ...
      const genTextSection = () => {
        return <div class="g-css-customizer-design-panel-section-content">
          {
            [genTextSectionCombobox(),
              genTextSectionSelect(),
              genTextSectionInput(designPropertyList.value.text[2], designData.value.text.fontSize),
              genTextSectionInput(designPropertyList.value.text[3], designData.value.text.lineHeight),
              genTextSectionInput(designPropertyList.value.text[4], designData.value.text.letterSpacing)
            ]}
        </div>
...
<style scoped lang="scss">
	...
	.g-css-customizer-design-panel {
		...
		&-effect-shadow-editor {
			...
			grid-template-areas: "header header header" "blurtitle xtitle ytitle" "blurinput xinput yinput" "colorinput alphainput .";
		...
		&-effect-blur-editor {
			...
			grid-template-areas: "header header" "blurtitle blurinput";
