...
function _getFontAweSomeIconSource() {
  ...
  function _getFontAwesomeCategories(iconSrc, type) {
    return _.map(FontAwesomeCategories, category => ({
      name: category.name,
      icons: _.filter(_.map(category.icons, ico => _.includes(iconSrc, ico) ? {
        name: ico,
        value: `${type} fa-${ico}`
  ...
  // brands doesn't divide into categories
  function _getBranchIcons() {
    return _.map(fab, val => ({
      name: val,
      value: `fab fa-${val}`,
    }))
  }
  ...
  return [
    {
      name: 'Font Awesome 5 Brands',
      source: 'https://fontawesome.com/',
      totalIcons: fab.length,
      categories: [{
        name: 'brands',
        icons: _getBranchIcons()
      }]
    },
    {
      name: 'Font Awesome 5 Solid',
      source: 'https://fontawesome.com/',
      totalIcons: fas.length,
      categories: _getFontAwesomeCategories(fas, 'fas')
    },
    {
      name: 'Font Awesome 5 Regular',
      source: 'https://fontawesome.com/',
      totalIcons: far.length,
      categories: _getFontAwesomeCategories(far, 'far')
    }
  ]
}
