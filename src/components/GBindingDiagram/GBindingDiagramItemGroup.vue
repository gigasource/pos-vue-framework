<script>
  ...
  import { ref, computed, provide, inject, onMounted } from '@vue/composition-api';
  ...
  export default {
    ...
    setup (props, context) {
      ...
      function dragEnd() {
			  const group = context.refs.group
				const top = parseInt(window.getComputedStyle(group).getPropertyValue('top'))
        const left = parseInt(window.getComputedStyle(group).getPropertyValue('left'))
			...
			const genRemoveButton = () => {
			  return <g-btn icon small vOn:click={() => context.emit('removeGroup', props.path, props.localPath)}>
          <g-icon color="red" size="16">fas fa-times-circle</g-icon>
        </g-btn>
			}

      const genBindingDiagramItemGroup = () => {
				return <div class="g-binding-item-group b-grey" style={styles.value} vOn:mousedown={onMousedown} ref="group">
            ...
            return <g-binding-diagram-item value={item}
																					 path={model.path}
																					 vOn:connected={connect}
																					 vOn:disconnected={disconnect}
																					 vOn:dragEnd={dragEnd}
